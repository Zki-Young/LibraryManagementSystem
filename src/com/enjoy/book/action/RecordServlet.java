...
public class RecordServlet extends HttpServlet {
    ...
    /**
     * /record.let?type=add&mid=xx&ids=xx   图书借阅
     * /record.let?type=queryback&idn=xx    根据会员的身份证号查询会员信息及借阅信息
     * /record.let?type=back&mid=xx&ids=xx  图书归还
     * /record.let?type=keep&id=xx          书籍续借
     * /record.let?type=doajax&typeId=xx&keyword=xx     ajax查询
     *
     * @param req
     * @param resp
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch(type){
            case "add" :
                ...
                if(count>0){
                    out.print("<script>alert('图书借阅成功');location.href='main.jsp'</script>");
                } else {
                    out.print("<script>alert('图书借阅失败');location.href='main.jsp'</script>");
                }
                break;
            case "queryback" :
                //1. 获取会员的身份证号
            ...
            case "doajax":
                ...
                //没有输入：keyword：空串"" null:没有找到 kewword
