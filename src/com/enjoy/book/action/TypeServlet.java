...
public class TypeServlet extends HttpServlet {
     ...
     * /type.let?type=modifypre     修改预备界面（request->转发->type_modify.jsp）
     * /type.let?type=modify    修改（表单）
     * /type.let?type=remove&id=xx  删除（获取删除的类型编号）
     * /type_list.jsp 查询（所有的类型数据：当项目运行时（监听器），将数据读，放到application对象）
     * servlet：
     *  request ：同一个请求当中传输信息
     *  session ：同一个会话（用户）
     *  application ：同一个运行项目
    ...
    private void modifyPre(HttpServletRequest req, HttpServletResponse resp, PrintWriter out, ServletContext application) throws ServletException, IOException {
        ...
        //把type存到req中，同一个功能，req，（session，applicaton太大，不合适）
