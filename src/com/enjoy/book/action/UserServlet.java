    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch(method){
            case "login":
                ...
                //调用UserBiz的getUser方法，根据用户名和密码获取对应用的用户对象
                User user = userBiz.getUser(name, pwd);

                //判断用户对象是否为null
                if(user==null){//如果是null，表示用户名或密码不正确，提示错误，回到登录界面
                    out.println("<script>alert('用户名或密码不存在');location.href = 'login.html';</script>");
                } else {//非空：表示登录成功，将用户兑现保存到session中，提示登录成功后，将页面跳转到index.jsp
...
//                    out.println("<script>alert('登录成功');location.href = 'index.jsp';</script>");
            ...
            case "exit" :
                ...
                //清除session
                session.invalidate();
                //跳转到login.html（框架中需要回去） top.jsp -> parent -> index.jsp
